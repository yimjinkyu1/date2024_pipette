...
def split_data(input_df):
    ...
    mbs_bwd_comp_time = x_df['avg_bwd_comp_time'] / x_df['gas']   
    columns = ['mbs_bwd_comp_time']
    mbs_bwd_comp_time_df = pd.DataFrame(mbs_bwd_comp_time, columns=columns)
    

    x_df = x_df[x_df.columns[:0-9]] 
    x_df = pd.concat([x_df, mbs_df], axis=1)

    y_df = mbs_bwd_comp_time_df['mbs_bwd_comp_time']
    
    return x_df, y_df

json_file_path = 'merge_multi_profile_gpt_a100.json'
...
real_json_file_path =  'merge_profile_real_result_a100.json'
...
for epoch in range(epochs):
    ...
    # Epoch마다 손실 출력
    if (epoch+1) % 10 == 0:
        print(f"Epoch {epoch+1}/{epochs},bwd Loss: {loss.item():.4f}, Learning Rate: {scheduler.get_last_lr()[0]:.5f}")
end_time = datetime.now()
model_train_time = end_time - start_time
model_train_time_sec = model_train_time.total_seconds()
print(f"bwd time model train time(a100) (sec) : {model_train_time_sec}")

# 테스트 데이터에 대한 평가 (선택적)
with torch.no_grad():
    test_predictions = model(X_test_tensor)
    test_loss = criterion(test_predictions, y_test_tensor)
    print(f"Test bwd Loss: {test_loss.item():.4f}")
    
    
with torch.no_grad():
    test_predictions = model(X_real_test_tensor)
    test_loss_real = criterion(test_predictions, y_real_test_tensor)
    print(f"Test bwd Loss real: {test_loss_real.item():.4f}")
...
plt.xlabel("Predict_Backward_Time(mbs)")
plt.ylabel("Backward_Time(mbs)")
plt.grid(True)

# 그림 파일로 저장
filename = "mbs_bwd_time_spearman_correlation_plot_origin_a100.png"
...
torch.save(model, 'model_predict_bwd_time_a100_torch_origin_{}_{}.pkl'.format(args.origin,args.device))

joblib.dump(scaler, 'standard_scaler_model_real_result_a100_torch_origin_{}_{}.pkl'.format(args.origin,args.device))

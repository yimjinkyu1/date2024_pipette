...
def split_data(input_df):
    ...
    y_df = input_df['gpu_mem_max']
    x_df = input_df[input_df.columns[:-7]]
    ...
    mini_bs= x_df['gbs'] // x_df['dp'] 
    columns = ['mini_bs']
    mbs_df = pd.DataFrame(mini_bs, columns=columns)
...
for epoch in range(epochs):
    ...
    # Epoch마다 손실 출력
    if (epoch+1) % 10 == 0:
        print(f"Epoch {epoch+1}/{epochs}, Loss: {loss.item():.4f}, Learning Rate: {scheduler.get_last_lr()[0]:.5f}")
end_time = datetime.now()
model_train_time = end_time - start_time
model_train_time_milli = model_train_time.total_seconds()
print(f"gpu mem model train time(a100) : {model_train_time_milli}")

# 테스트 데이터에 대한 평가 (선택적)
with torch.no_grad():
    test_predictions = model(X_test_tensor)
    test_loss = criterion(test_predictions, y_test_tensor)
    print(f"Test Loss real: {test_loss.item():.4f}")
    
    
# with torch.no_grad():
#     test_predictions = model(X_real_test_tensor)
#     test_loss_real = criterion(test_predictions, y_real_test_tensor)
#     print(f"Test Loss: {test_loss_real.item():.4f}")
    

predict_y = test_predictions.cpu()
test_y = y_test_tensor.cpu()
...
torch.save(model, "model_predict_gpu_mem_a100_torch_origin_{}_{}.pkl".format(args.origin,args.device))

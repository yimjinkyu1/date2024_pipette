...
print(f"gpu mem model train time(v100) (sec) : {model_train_time_sec}")
...
# 그래프 그리기
#plt.scatter(bmt_ouput_y, bmt_pred_y, label=f'Spearman correlation: {corr:.3f}')
plt.scatter(test_y, predict_y)
plt.title(f"Spearman Correlation: {corr:.3f}")
plt.xlabel("Predict_GPU_Max_Memory")
plt.ylabel("GPU_Max_Memory")
plt.grid(True)

# 그림 파일로 저장
filename = "gpu_mem_spearman_correlation_plot.png"
...
torch.save(model, 'model_predict_gpu_mem_v100_torch_origin_{}_{}.pkl'.format(args.origin,args.device))

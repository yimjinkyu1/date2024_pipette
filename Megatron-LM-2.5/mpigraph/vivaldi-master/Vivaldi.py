...
class Vivaldi():
	...
	def run(self):
		...
		for i in range(iters):
			...
			for node, neighbor in enumerate(self.graph.getAdjacentList()):
				#print (node, ",", neighbor)
				#print (self.graph.getAdjacent(node))

				neighbors = self.graph.getAdjacent(node)
    
				#print(neighbors)
    			#print('neighbors ', random.choice(neighbors))

				random_neighbors = []

				for _ in range(self.configuration.getNumNeighbors()):
					random_neighbors.append(random.choice(neighbors))
	...
	def getRTTGraph(self):
		...
		for nid, tic in enumerate(self.graph.getAdjacentList()):
			#print(nid, ",", tic)
			prediction[nid] = [0] * self.configuration.getNumNodes()
			neighbors = self.graph.getAdjacent(nid)
